...
// utils/map/createClusterRenderer.js
// Leaflet version.
//
// Usage: pass the result of this function as `iconCreateFunction`
// to L.markerClusterGroup({ iconCreateFunction: createClusterRenderer(...) })
//
// The Google Maps version returned { render: fn } for MarkerClusterer.
// Leaflet needs a plain function: (cluster) => L.Icon

import ReactDOMServer from "react-dom/server";
import { Briefcase, FileText, MapPin, Newspaper, Heart } from "lucide-react";

const TYPE_CONFIGS = {
  page:      { color: "#2563EB", icon: FileText,   shape: "marker" },
  job:       { color: "#059669", icon: Briefcase,  shape: "square" },
  news:      { color: "#9333EA", icon: Newspaper,  shape: "marker" },
  leads:     { color: "#9333EA", icon: Newspaper,  shape: "marker" },
  event:     { color: "#DC2626", icon: MapPin,     shape: "marker" },
  myfriends: { color: "#F59E0B", icon: Heart,      shape: "circle" },
};
...
export const createClusterRenderer = (
  ...
  // ── Returns a Leaflet iconCreateFunction ────────────────────────────────────
  // Called by leaflet.markercluster for every cluster bubble.
  return (cluster) => {
    ...
    if (currentZoom <= 3) {
      baseSize = { width: 36, height: 42 }; iconSize = 16; fontSize = "8";  badgeRadius = 8;  strokeWidth = 2;
    } else if (currentZoom <= 6) {
      baseSize = { width: 44, height: 52 }; iconSize = 20; fontSize = "9";  badgeRadius = 10; strokeWidth = 2.5;
    } else {
      baseSize = { width: 56, height: 66 }; iconSize = 26; fontSize = "11"; badgeRadius = 12; strokeWidth = 3;
    }

    if (finalShape === "square") {
      const d = Math.max(baseSize.width, baseSize.height) * 0.8;
      baseSize = { width: d, height: d };
    } else if (finalShape === "circle") {
      const d = Math.max(baseSize.width, baseSize.height) * 0.9;
      baseSize = { width: d, height: d };
    }
    ...
    for (const m of childMarkers) {
      if (!m.options._allRead) { allMarkersRead = false; break; }
    }
    ...
    const grayscaleId = `cg-${uid}`;

    const getShapePath = () => {
      if (finalShape === "square") {
        return `<rect x="4" y="20" width="${baseSize.width - 8}" height="${baseSize.height - 8}" rx="4" ry="4" fill="${clusterFinalColor}" stroke="white" stroke-width="${strokeWidth}" filter="url(#${shadowId})${allMarkersRead ? ` url(#${grayscaleId})` : ''}"/>`;
      } else if (finalShape === "circle") {
        const r  = baseSize.width / 2 - 4;
        const cx = baseSize.width / 2;
        const cy = baseSize.height / 2 + 4;
        return `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${clusterFinalColor}" stroke="white" stroke-width="${strokeWidth}" filter="url(#${shadowId})${allMarkersRead ? ` url(#${grayscaleId})` : ''}"/>`;
      } else {
        return `<path d="M ${baseSize.width/2},4 C ${baseSize.width*0.9},4 ${baseSize.width*0.95},${baseSize.height*0.5} ${baseSize.width/2},${baseSize.height-2} C ${baseSize.width*0.05},${baseSize.height*0.5} ${baseSize.width*0.1},4 ${baseSize.width/2},4 Z" fill="${clusterFinalColor}" stroke="white" stroke-width="${strokeWidth}" filter="url(#${shadowId})${allMarkersRead ? ` url(#${grayscaleId})` : ''}"/>`;
      }
    };

    const getIconTransform = () => {
      if (finalShape === "square")  return `translate(${baseSize.width/2 - iconSize/2}, ${baseSize.height/2 - iconSize/2 + 16})`;
      if (finalShape === "circle")  return `translate(${baseSize.width/2 - iconSize/2}, ${baseSize.height/2 - iconSize/2 + 4})`;
      return `translate(${baseSize.width/2 - iconSize/2}, ${baseSize.height * 0.2})`;
    };

    const getBadgePos = () => {
      if (finalShape === "square")  return { cx: baseSize.width - badgeRadius/2 + 4, cy: 20 + badgeRadius/2 };
      if (finalShape === "circle")  return { cx: baseSize.width - badgeRadius/2 + 4, cy: 4  + badgeRadius/2 };
      return { cx: baseSize.width - (badgeRadius + 2), cy: badgeRadius + 2 };
    };

    const bp  = getBadgePos();
    const w   = baseSize.width  + 8;
    const h   = baseSize.height + 8;
    ...
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}">
        <defs>
          ...
          ${allMarkersRead ? `<filter id="${grayscaleId}"><feColorMatrix type="saturate" values="0"/></filter>` : ''}
        </defs>
        ${getShapePath()}
        <g transform="${getIconTransform()}"${allMarkersRead ? ` filter="url(#${grayscaleId})"` : ''}>${iconSvg}</g>
        <circle cx="${bp.cx}" cy="${bp.cy}" r="${badgeRadius}" fill="${allMarkersRead ? '#9CA3AF' : '#EF4444'}" stroke="white" stroke-width="2"/>
        <text x="${bp.cx}" y="${bp.cy + parseInt(fontSize)/3}" font-family="Arial,sans-serif" font-size="${fontSize}" font-weight="bold" text-anchor="middle" fill="white">${count}</text>
      </svg>`;

    // Anchor calculation
    let anchorX, anchorY;
    if (finalShape === "square" || finalShape === "circle") {
      anchorX = w / 2; anchorY = h / 2 + 10;
    } else {
      anchorX = baseSize.width / 2; anchorY = baseSize.height - 4;
    }

    // Return Leaflet DivIcon (works with leaflet.markercluster iconCreateFunction)
    // We use dynamic import via the global L set during map boot
    const L = typeof window !== "undefined" ? window._leaflet_L : null;
    if (!L) {
      // Fallback plain object — will be ignored gracefully
      return { html: `<div>${svg}</div>`, className: '' };
    }
