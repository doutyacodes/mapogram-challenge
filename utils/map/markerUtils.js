...
export const createPostCategoryMarkerIcon = (categoryData, postCount = 0, post, isRead = false, needsBlinking = false) => {
...
// Add blinking background effect if needed
const blinkingBackground = needsBlinking ? `
  <!-- Blinking red background circle -->
  <circle cx="24" cy="35" r="26" fill="#ff0000" opacity="0.6">
...
export const groupPostsByLocation = (postsItems, threshold = 0.00005) => {
  const groupedPosts = [];
  ...
  postsItems.forEach((post) => {
    ...
    // Try to find an existing cluster near this post
    const existingCluster = groupedPosts.find(group => {
      const dLat = Math.abs(group.lat - lat);
      const dLng = Math.abs(group.lng - lng);
      return dLat < threshold && dLng < threshold;
    });

    if (existingCluster) {
      existingCluster.posts.push(post);
    } else {
      groupedPosts.push({
        lat,
        lng,
  ...
  // Sort posts in each cluster by created_at
  groupedPosts.forEach(group => {
    group.posts.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
  });

  // Convert back to your required structure (lat,lng keys)
  const groupedPostsMap = {};
  groupedPosts.forEach(group => {
    const key = `${group.lat},${group.lng}`;
    groupedPostsMap[key] = group.posts;
  });
