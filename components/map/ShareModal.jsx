...
function ShareModal({getCurrentPostItem}) {

    /*Share Oprions */
    const generateShareUrl = (post, lat, lng) => {
        ...
        return `${baseUrl}?postId=${post.id}&lat=${formattedLat}&lng=${formattedLng}&preview=true`;
    };

    const handleShareClick = (post) => {
        const url = generateShareUrl(post, selectedLocation.lat, selectedLocation.lng);
        setShareUrl(url);
        setShowShareModal(true);
    };

    const handleShare = async (platform) => {
        const post = getCurrentPostItem();
        const title = `${post.title} - ${post.source_name}`;
