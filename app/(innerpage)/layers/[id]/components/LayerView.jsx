...
export default function PageView({layerId}) {
  ...
  const fetchPostsData = useCallback(async (bounds, layerId, isAutoRefresh = false, filters = {}, quickFilter = 'all') => {
  try {
    if (!isAutoRefresh) {
      setIsLoading(true);
    }
    
    // Check if this is a classifieds layer
    const isClassifiedsLayer = layerType === 'classifieds';
    
    let url = isClassifiedsLayer ? '/api/classifieds' : '/api/layer';
    const params = new URLSearchParams();
    
    // Geographic bounds (if you want to re-enable them later)
    // if (bounds) {
    ...
    // Add quick filter parameter (only for non-classifieds)
    if (!isClassifiedsLayer && quickFilter && quickFilter !== 'all') {
  ...
  const handleApplyFilters = (filters) => {
    ...
    fetchPostsData(null, layerId, false, filters);
  };
  ...
  useEffect(() => {
    if (layerId && layerType) {
      fetchPostsData(null, layerId);
    }
  }, [layerType, layerId, fetchPostsData]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (availableLanguages.length > 0) {
        fetchPostsData(null, layerId, true); // true = auto-refresh
        setLastFetchTime(Date.now());
      }
    }, 30000); // 30 seconds

    return () => clearInterval(interval);
  }, [fetchPostsData, layerId]);
