...
//   window._leaflet_L = L; // expose for createClusterRenderer / handleClusterClick
...
// // ── Helper: Google-style icon descriptor → Leaflet icon ───────────────────────
// function toLeafletIcon(descriptor) {
//   if (!descriptor || !L) return new L.Icon.Default();
//   const url  = descriptor.url;
//   const size = descriptor.scaledSize
//     ? [descriptor.scaledSize.width, descriptor.scaledSize.height]
//     : [48, 70];
//   const anchor = descriptor.anchor
//     ? [descriptor.anchor.x, descriptor.anchor.y]
//     : [size[0] / 2, size[1]];
...
//   // ── STATE — NO CHANGE ────────────────────────────────────────────────────────
//   // PASTE HERE: all useState declarations exactly as in the original file,
//   // from `const [postsItems, setPostsItems]` all the way through
//   // `const [showUploadModal, setShowUploadModal]` and `const [uploading, ...]`
//   // and `const [uploadSuccess, ...]` — nothing in that block changes.
//   // ───────────────────────────────────────────────────────────────────────────
...
//   // ── Leaflet-specific refs ─────────────────────────────────────────────────
//   const mapContainerRef   = useRef(null);
//   const mapInstanceRef    = useRef(null);
//   const clusterGroupRef   = useRef(null);
//   const markersMapRef     = useRef(new Map());        // locationKey → L.Marker
//   const geofenceLayersRef = useRef([]);               // polygon layers for cleanup
//   const discoveryMarkersRef = useRef([]);             // district centre dot markers
//   const tourismLeafletMarkersRef = useRef([]);        // tourism item markers
//   const [leafletReady, setLeafletReady]  = useState(false);

//   // Popup portal DOM nodes
//   const mainPopupDomRef    = useRef(null);
//   const mainPopupRef       = useRef(null);   // L.Popup instance
...
//       // Cluster group — iconCreateFunction comes from createClusterRenderer
...
//   // ── 2. Marker management (posts + registrations) ─────────────────────────────
//   useEffect(() => {
...
//       const googleIcon   = createPostCategoryMarkerIcon(categoryData, postsAtLocation.length, mainPost, allPostRead);
//       const icon         = toLeafletIcon(googleIcon);
...
//   // ── 3. Geofence polygons + overlay + district centre dots ────────────────────
//   useEffect(() => {
...
//       const ringToLatLngs = (ring) => ring.map(coord => [coord[1], coord[0]]);

//       const rings = geometry.type === 'Polygon'
//         ? geometry.coordinates
//         : geometry.coordinates.flatMap(poly => poly); // MultiPolygon → flatten
...
//       // Overlay: world - district hole (grey dim on unselected areas)
//       // Build world ring + reversed district ring for "punch-hole" effect
...
//       // Rebuild overlay whenever selected district changes — done once per effect
//       // (handled below after loop)

//       // District centre dot (shown when no district selected)
//       if (!selectedDistrict) {
...
//     // Grey overlay (world minus selected/all districts)
//     const worldRing = [[85,-180],[85,180],[-85,180],[-85,-180]];
//     const holesToDraw = selectedDistrict
//       ? geofenceData.features.filter(f => (f.properties.name || f.properties.district)?.toLowerCase() === selectedDistrict.toLowerCase())
//       : geofenceData.features;
...
//         ? [feature.geometry.coordinates[0].map(c => [c[1], c[0]]).reverse()]
//         : feature.geometry.coordinates.map(poly => poly[0].map(c => [c[1], c[0]]).reverse());
//     });
...
//     if (hasCoords) {
//       if (selectedDistrict) {
//         const selFeature = geofenceData.features.find(f => (f.properties.name || f.properties.district)?.toLowerCase() === selectedDistrict.toLowerCase());
//         if (selFeature) {
...
//           map.fitBounds(selBounds, { padding: [80, 40] });
//         }
//       } else {
//         map.fitBounds(allBounds, { padding: [80, 40] });
//       }
//     }
...
//     visibleTourismMarkers.forEach((marker, index) => {
...
//   // ── 5. Main post/registration popup via portal ───────────────────────────────
...
//       return;
//     }
//     if (mainPopupRef.current) return; // portal updates in-place
...
//       maxWidth:    320,
//       closeButton: false,
//       className:   'leaflet-tourism-popup',
//     })
...
//   // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [leafletReady, activeCategoryMarker]);
...
//   // Pan to district center when it changes
//   useEffect(() => {
//     if (mapInstanceRef.current && districtCenter) {
//       mapInstanceRef.current.setView([districtCenter.lat, districtCenter.lng],
...
//   // 14. Cleanup on unmount (useEffect return cleanup)

//   // ── UPDATED: Distance / auto-popup — replaces window.google.maps.geometry ───
//   useEffect(() => {
...
//   // ── UPDATED: isWithinRadius — replaces window.google.maps.geometry ──────────
//   const isWithinRadius = useCallback((item) => {
//     if (!userLocation || !item || !item.position) return false;
...
//   // ── ALL NON-MAP CALLBACKS — NO CHANGE ───────────────────────────────────────
//   // PASTE HERE exactly as in the original:
//   //
//   //  - fetchDistrictChallenges (inside useEffect) — NO CHANGE
//   //  - handleMarkerClick   — NO CHANGE
//   //  - handleNextPost / handlePrevPost — NO CHANGE
//   //  - handleLikePost      — NO CHANGE
//   //  - getCurrentPostItem  — NO CHANGE
//   //  - handleRegistrationMarkerClick / handleNextRegistration / etc — NO CHANGE
//   //  - Tourism handlers: handleAcceptItem, handleDenyItem, handleQRScan, handleShowRoute — NO CHANGE
//   //  - filteredDistricts   — NO CHANGE
//   //
//   // ────────────────────────────────────────────────────────────────────────────
...
//     try {
//       const response = await fetch('/api/posts/like', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ postId }) });
//       if (response.ok) {
//         const data = await response.json();
...
//   // ── Tourism memos — NO CHANGE ────────────────────────────────────────────────
//   // PASTE HERE exactly from original:
//   //  - districtCenter (useMemo)
//   //  - visibleTourismMarkers (useMemo)
...
//       {/* ── All non-map UI overlays — NO CHANGE ────────────────────────────────
//           PASTE HERE exactly from original:
//           1. Filter Controls (top-right absolute div with PostFilterPanel / MobileFilterDropdown / ResetMapButton)
//           2. Mobile Reset Button
//           3. Tourism District label (top-left)
//           4. Accepted Items Stack (left side)
//           5. Tourism Category Filter (right side vertical)
//           6. "No posts found" overlay
//           ──────────────────────────────────────────────────────────────────── */}
...
//               return (
//                 /* PASTE the exact InfoWindowF inner JSX from the original here —
//                    the <div className="bg-white rounded-[2rem] ..."> block.
//                    Nothing inside it changes. */
//                 <div className="bg-white rounded-[2rem] overflow-hidden font-sans relative w-[240px] sm:w-[280px]">
...
//           PASTE HERE exactly from original:
//           1. Detail Sliding Modal (showDetailModal)
//           2. QR Scan Modal (showQRModal)
//           3. Upload Media Modal (showUploadModal)
//           4. PostRegistrationModal
//           5. Loading overlay
//           6. Error message
//           ──────────────────────────────────────────────────────────────────── */}
